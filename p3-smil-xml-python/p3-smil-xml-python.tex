...
%Nota: no hace falta incluir \texttt{practica3-chistes.py} ni \texttt{chistes2.xml} en el repositorio git, ya que este ejercicio es simplemente para familiarizarse con el uso de SAX.
...
%\finejercicio
  ...
  \item En el fichero \texttt{smallsmilhandler.py}, crea una clase llamada \texttt{SmallSMILHandler} que herede de la clase \texttt{ContentHandler}. Las etiquetas SMIL que deberá reconocer nuestra clase son las siguientes (se enumera junto con las etiquetas los atributos que se han de tener también en cuenta):
  \begin{itemize}
  ...
  \end{itemize}
	...
	Una vez hayas acabado con este ejercicio, no olvides hacer \texttt{commit} del fichero \texttt{smallsmilhandler.py}.
...
\finejercicio
  ...
  \begin{itemize}
    \item Leer el fichero SMIL \texttt{karaoke.smil} que se puede encontrar en Moodle.
    \item Mostrar por pantalla un listado ordenado de las etiquetas y de los pares atributo-valor (para aquéllos que tengan un valor asignado), cada uno en una línea, separados por tabuladores ('\t') y sin espacios antes y después del signo igual, tal y como se muestra a continuación:
\begin{verbatim}
...
\finejercicio
  ...
  \item Modifica el programa principal \texttt{karaoke.py} para que toda la funcionalidad descrita en los ejercicios 4, 5 y 6 sea orientada a objetos, específicamente en una clase llamada \texttt{KaraokeLocal}. Esta clase deberá tener los siguientes métodos:
  \begin{itemize}
  ...
  \begin{itemize}
    \item Comprobará que no hay errores en la invocación por parte del usuario (véase ejercicio 6). 
   ...
   \end{itemize}
Nótese que el programa sólo deberá imprimir por pantalla mediante el método \texttt{print\_list} y en las llamadas a \texttt{wget} en \texttt{do\_local}. El resto del programa, al entregar la práctica, no debería contener más prints.

Modifica el programa principal \texttt{karaoke.py} para que permita al usuario indicar como argumento del programa el fichero SMIL fuente (que posiblemente contenga referencias a recursos remotos). La invocación del programa deberá ser de esta manera (¡\texttt{fichero\_fuente.smil} es un parámetro!):
\begin{verbatim}
$ python karaoke.py fichero_fuente.smil
\end{verbatim}
	En el caso de que el usuario no incluya los parámetros suficientes, el programa devolverá:
\begin{verbatim}
	      Usage: python karaoke.py src_file.smil
\end{verbatim}
		Una vez hayas acabado con este ejercicio, no olvides hacer add y commit del fichero \texttt{karaoke.py}.

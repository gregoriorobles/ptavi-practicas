...
\finejercicio
  ...
  \begin{itemize}
    \item Comprobará que no hay errores en la invocación por parte del usuario (véase ejercicio 5). 
...
Modifica el programa principal \texttt{karaoke.py} para que permita al usuario indicar como argumento del programa el fichero SMIL fuente (que posiblemente contenga referencias a recursos remotos). La invocación del programa deberá ser de esta manera\footnote{Date cuenta de que \texttt{fichero\_fuente.smil} es un parámetro que se ha de pasar por línea de comandos.}:
\begin{verbatim}

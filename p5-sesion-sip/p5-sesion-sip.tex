...
Se ha capturado una sesión SIP (archivo \texttt{sip.cap.gz}), que se puede abrir con \texttt{Wireshark} (desde la shell \texttt{\$ wireshark sip.cap.gz}; recuerda que puedes utilizar el tabulador en la shell para autocompletar). Se pide rellenar las cuestiones que se plantean en este guión en el fichero \texttt{p5.txt} que encontrarás también en el repositorio. 
  ...
  \begin{itemize}
    \item Filtra por \texttt{sip} para conocer cuándo se envían paquetes con SIP. ¿En qué segundos tienen lugar?
    \item Y los paquetes con RTP, ¿entre qué segundos se envían?
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item ¿Qué pasa en el caso de \texttt{35.162.64.86.in-addr.arpa}?
  \end{itemize}
  ...
  \begin{itemize}
    \item ¿Qué URL se pide? 
    \item ¿Qué \texttt{user agent} (UA) la está pidiendo? 
    \item ¿Qué devuelve el servidor? 
    \item Si lanzamos el navegador web, por ejemplo, \texttt{Mozilla Firefox}, y apuntamos a la URL, ¿qué recibimos? Entonces, ¿qué IP es la que podemos ver en la captura? 
  \end{itemize}
  ...
  \begin{itemize}
    \item ¿Por qué crees que se hace uso de este protocolo en esta captura? 
    \item ¿Estamos tras un \texttt{full-cone NAT}? (en el diagrama de la Wikipedia verás el algoritmo que se sigue)
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item Además de REGISTER, existen otras instrucciones SIP, ¿qué instrucciones SIP entiende el UA?
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item ¿Podrías identificar las diferencias entre el primer intento y el segundo de registro?
  ...
  \begin{itemize}
    ...
    \item ¿En qué trama(s)?
    \item ¿Por qué se envía varias veces?
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item ¿En qué se diferencia con la instrucción que se utilizó con anterioridad (al principio de la sesión, en el ejercicio 9)?
  \end{itemize}
...
Captura una sesión SIP de una conversación con el número SIP \texttt{sip:9999@qxip.net}. Tendrás que dar de alta una cuenta en \texttt{www.ekiga.net} con el navegador. Recuerda también que has de comenzar a capturar tramas antes de arrancar Ekiga para ver todo el proceso en el que participa SIP. Para realizar los siguientes ejercicios, deberás crearte una cuenta en \texttt{ekiga.net} y darte de alta la lanzar la aplicación.
  ...
  \item Filtra por los paquetes SIP de la captura y guarda únicamente los paquetes SIP como sip.libpcap. Abre el fichero guardado para cerciorarte de que lo has hecho bien. Deberás guardarlo en un archivo de nombre \texttt{sip.libpcap} y añadirlo al repositorio.
...
\begin{itemize}
  ...
  \begin{enumerate}
    ...
    \item Un archivo con una captura filtrada en \texttt{sip.libpcap} (véanse ejercicios 18 a 20).

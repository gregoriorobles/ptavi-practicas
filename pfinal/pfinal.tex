...
\begin{enumerate}
  \item uaclient.py (cliente del UA)
  \item uaserver.py (servidor del UA)
  \item proxy\_registrar.py (servidor que hace funciones de \texttt{proxy} y \texttt{Registrar})
  \item ua1.xml (fichero de configuración del UA)
  \item ua2.xml (fichero de configuración del UA)
  \item pr.xml (fichero de configuración del proxy/registrar)
  \item llamada.libpcap (captura de una llamada exitosa)
  \item error.libpcap (captura de una llamada con error)
  ...
  \item Otros: README.md, .gitignore, .git, check-pfinal.py, cancion.mp3, mp32rtp
\end{enumerate}
